...
const getAgendaPrincipalQueryKey = (unitId?: string, dateISO?: string) =>
  ['agenda', 'principal', unitId ?? 'no-unit', dateISO ?? 'no-date'] as const;

const isFreeMorningStatus = (status: unknown) =>
  status === 'LIVRE' ||
  status === '4 horas manhã' ||
  status === '6 horas' ||
  status === '8 horas';

const isFreeAfternoonStatus = (status: unknown) =>
  status === 'LIVRE' ||
  status === '4 horas tarde' ||
  status === '6 horas' ||
  status === '8 horas';

export const useAgendaPrincipal = (
  ...
  const [selectedProfDetails, setSelectedProfDetails] =
    useState<AgendaDisponibilidade | null>(null);
  const [filterSemProfissional, setFilterSemProfissional] = useState(false);
  const [activeMetricPeriod, setActiveMetricPeriod] = useState<
    'd7' | 'd30' | 'geral'
  >('d7');
  ...
  const agendaEnabled = Boolean(
    selectedUnit?.id && selectedUnit.id !== 'ALL' && selectedUnit.unit_code,
  );
    ...
    queryFn: async () => {
      ...
      const livresProfsFix = (livresRaw || []).filter((lp) => {
        const hasAtendimento = apts.some((a) => {
          const aDate =
            typeof a.DATA === 'string' && a.DATA.includes('T')
              ? a.DATA.split('T')[0]
              : a.DATA;
          const lpNome = (lp.profissional?.nome || '').trim().toLowerCase();
          const atProf = (a.PROFISSIONAL || '').trim().toLowerCase();
          return (
            aDate === dateISO && Boolean(a.PROFISSIONAL) && atProf === lpNome
          );
        });

        const isBlocked =
          lp.status_manha === 'RESERVA' ||
          lp.status_tarde === 'RESERVA' ||
          lp.status_manha === 'CANCELOU' ||
          lp.status_tarde === 'CANCELOU' ||
          lp.status_manha === 'FALTOU' ||
          lp.status_tarde === 'FALTOU';
        ...
        if (isToday) {
          ...
          const isManhaSolo =
            isFreeMorningStatus(lp.status_manha) &&
            !isFreeAfternoonStatus(lp.status_tarde);
          if (isManhaSolo && currentHour >= AGENDA_TURNO_DIVISOR_HOUR)
            return false;
        }
      ...
      const uniqueLivres = Array.from(
        livresProfsFix
          .reduce((map: Map<string, AgendaDisponibilidade>, item) => {
            if (!map.has(item.profissional_id))
              map.set(item.profissional_id, item);
            return map;
          }, new Map())
          .values(),
      );
      ...
      uniqueLivres.sort((a, b) => {
        ...
        return (a.profissional?.nome || '').localeCompare(
          b.profissional?.nome || '',
        );
      });
      ...
      .on(
        ...
        () =>
          queryClient.invalidateQueries({
            queryKey: ['agenda', 'principal', selectedUnit.id],
          }),
      )
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'atendimentos',
          filter: `unidade=eq.${selectedUnit.id}`,
        },
        () =>
          queryClient.invalidateQueries({
            queryKey: ['agenda', 'principal', selectedUnit.id],
  ...
  const loadProfissionalMetrics = async (profId: string, profNome: string) => {
    try {
      const response = await fetch(
        `${window.location.origin}/api/metrics/professional?id=${profId}&nome=${encodeURIComponent(profNome)}&unit_id=${selectedUnit?.id}`,
      );
